...
_fail() {
    ...
    _colorize red "Please check build.log and if you need help check out the team discord @ 'https://discordapp.com/invite/5Buudya'" && echo
...
_set_wd() {
    if [ -d "$PWD/.git" ] && [ -f "$PWD/Makefile" ]; then
        ...
        mkdir bloc
        git clone -b master -q https://github.com/bloc/bloc bloc   >>build.log 2>&1 || _fail "Unable to clone git repository. Please see build.log for more information"
        cd bloc
    fi
}
...
_note "BLOC Multi_Installer v1.0 (pepperoni)"
_colorize green " IN BLOC WE TRUST" && echo
