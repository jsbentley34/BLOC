...
Open the wallets with BLOC-service like so:

./BLOC-service -w walletA.wallet -p yourpass --rpc-password test --bind-port 8070
./BLOC-service -w walletB.wallet -p yourpass --rpc-password test --bind-port 8071
...
moveDecimal = 10000  # BLOC has 4 decimals so 10000 is the divide/multiply factor
minAmount = 10000 * moveDecimal  # min number for amount to xfer
maxAmount = 500000 * moveDecimal  # max number for amount to xfer
anonymity = 0
fee = 1 # atomic units, BLOC would be 0.0001 as the tx network fee
